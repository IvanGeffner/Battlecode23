...
public class MicroAttacker {
    ...
    class MicroInfo{
        ...
        int safe(){
            if (!canMove) return -1;
            if (launchersAttackRange > 1) return 0;
            if (launchersVisionRange > 1) return 1;
            if (possibleEnemyLaunchers > 1) return 2;
            return 3;
        }
        ...
        boolean isBetter(MicroInfo M){
            ...
            if (possibleEnemyLaunchers - canLandHit < M.possibleEnemyLaunchers - M.canLandHit) return true;
            if (possibleEnemyLaunchers - canLandHit > M.possibleEnemyLaunchers - M.canLandHit) return false;
